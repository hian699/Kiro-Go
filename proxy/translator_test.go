...
func TestParseModelAndThinking(t *testing.T) {
	tests := []struct {
		...
		// Format normalization: dot → dash for new versions without code changes.
		...
		{"thinking suffix on dot form", "claude-opus-4.8-thinking", "claude-opus-4-8", true},
		{"thinking suffix on dash form", "claude-sonnet-4-5-thinking", "claude-sonnet-4-5", true},
